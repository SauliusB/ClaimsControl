...
var oCONTROLS={ lbl: function(text) { return "<label class='dialog-form-label'>"+text+"</label>"; },
   ...
   appendLabel: function(p, t) { if(typeof p.label==='undefined') { return t; } else { return (p.label.type==="Top")?"<label class='toplabel'><span>"+p.label.txt+"</span>"+t+"</label>":"<label class='leftlabel'><span>"+p.label.txt+":</span>"+t+"</label>"; } },
   ...
   chk: function(p) { if(typeof p.Value==="string") { p.Value=((p.Value.search(/false/i)> -1)?0:1); } return "<label"+((p.label.classes)?" class='"+p.label.classes+"'":"")+"><input type='checkbox' "+this.basic(p)+((p.Value)?"checked='checked'":"")+"/>"+((p.label.txt)?p.label.txt:"")+"</label>"; },
   //{src:??,alt:??,onclickfn:??}
   ...
   UpdatableForm: function(frm) {
      ...
      $(frm).find('div.ExtendIt').each(function() {
         ...
         for(var prop in col) {
            ...
            else if(prop==='List') { $.extend(data_ctrl, col[prop]); data_ctrl.Type="List"; } //Listo propercius dedu vienam lygyje su kitais
            else if(prop==='classes') { classes+=' '+col[prop]; }
            else if(prop==='id') { id=col[prop]; }
            else if(prop==='attr') { attr=col[prop]; }
            else if(prop==='Value') { data_ctrl[prop]=col[prop]; Value=col[prop]; }
         }
         ...
         if(typeof col.List!=='undefined') {
            ...
            if(Type==='Integer'||Type==='Decimal') {
               ...
               eHTML+=oCONTROLS.txt({ "data_ctrl": data_ctrl, "text": Value, "title": eName, "classes": classes, "id": id, "attr": attr, "label": { "txt": eName, "type": col.labelType} });
            ...
            if(Type.search("Date")!== -1) {// Date DateNotMore DateNotLess DateNotMoreCtrl DateNotLessCtrl
               ...
               input=$(eHTML).prependTo(e).find('input:first');
               if(col.Plugin) {
                  for(var i=0; i<col.Plugin.length; i++) {
                     if(col.Plugin[i]==="datepicker") {
                        var CtrlOpt=(Type.search("More")!== -1)?{ minDate: '0', maxDate: '+3y +12m'}:{ minDate: '-10y -10m', maxDate: '0' };
                        input.datepicker(CtrlOpt);
                     } else { input[Plugin[i]](); }
                  }
               }
...
var oGLOBAL={
   ...
   GetNo: function(i) {
      ...
      var url="/Update/"+p.Action, updData={ "Action": p.Action, "DataToSave": p.DataToSave, "CallBack": p.CallBack, "Msg": p.Msg };
      //url=(p.url)?p.url:("/Update/"+p.Action); //Add/Edit.Delete
      log('<p style="color:green"><div>CallServer. Action:'+p.Action+'</div><div>DataToServer:'+JSON.stringify(p.DataToSave)+'</div><div> DataToClient:'+JSON.stringify(updData)+'</div> url:'+url+'</p>');
   ...
   ValidateForm: function(frm, DataToSaveAppend) { //Formos lauku validacija (pagal data-ctrl duomenis)
      ...
      $.each(frm.find(".UpdateField"), function(i, v) {
         ...
         if(e.data("ctrl")===undefined) { log("Nerasta data(ctrl),el: "+elDesc); return true; }
         log("OK,el: "+elDesc);

         //#region naujo value nustatymas
         ...
         //#endregion
         if(typeof e.data("ctrl").Type==='undefined') { log("<div style='color:red'>Nerastas elemento Tipas!</div>"); } else { var Type=e.data("ctrl").Type; }
         ...
         log("Element rez: val-"+val+", OldVal-"+OldVal+", Type-"+Type+", Save-"+Save);
      });

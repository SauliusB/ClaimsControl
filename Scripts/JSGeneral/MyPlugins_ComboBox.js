...
(function($) {
   ...
   .autocomplete({
      ...
      select: function(event, ui) {
         ...
         //$(input).rem
         if(opt.Editable) { //linko pridėjimas
            var t=input, newVal=t.data("newval"); // (t.data("newval"))?t.data("newval").replace("0", ""):"";
            if(!t.hasClass('alink')&&newVal) { t.addClass('alink').unbind('dblclick').bind('dblclick', function() { fnEDITFORM({ objName: opt.Source, id: newVal }); }) }
            else if(newVal) { t.unbind('dblclick').bind('dblclick', function() { fnEDITFORM({ objName: opt.Source, id: newVal }); }) }
            else if(t.hasClass('alink')&&!newVal) { t.removeClass('alink').unbind('dblclick'); }
         }
         if(opt.ListType!=="List") { input.removeClass("activeField"); }
      }, open: function() { if(opt.ListType!=="List") { if(!input.hasClass("activeField")) { input.addClass("activeField"); } } }
   })
         if(opt.Editable) {
            var val=input.data("newval"), p={ objName: opt.Source, id: (val)?val:0 };
            if(val) {
               input.addClass('alink').bind('dblclick', function() { fnEDITFORM(p); });
               //fnEDITFORM({ objName: opt.data_ctrl.DataSource, id: val })      reikia kist i function(){} nes priesingu atveju is karto paleidzia
            }
         }
         //---------------------------------------------------------------------------------------------------
         ...
         if(opt.Editable) {
            ...
            this.addButton({ title: "Pridėti naują", icon: "ui-icon-circle-plus", fn: function() { fnEDITFORM({ objName: opt.data_ctrl.DataSource, id: 0 }); }, NoCorners: false, green: true }, input);
         };

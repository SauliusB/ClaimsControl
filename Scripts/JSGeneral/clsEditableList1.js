...
clsEditableList.prototype={
   ...
   AddFieldControls: function(c) {
      ...
      for(var i=0; i<c.length; i++) {
         if(c[i].Markup!=undefined) {
            if(c[i].Markup.Type!=undefined) {
               var ctrl="$('#_"+c[i].FName+"')";
               if(c[i].Markup.Type=='DateCtrl') { js+=ctrl+".datepicker();"; }
               else if(c[i].Markup.Type=='DateNotLessCtrl') { js+=ctrl+".datepicker({minDate:'-0',maxDate:'+5y'});"; }
               else if(c[i].Markup.Type=='DateNotMoreCtrl') { js+=ctrl+".datepicker({minDate:'-5y',maxDate:'+0'});"; }
               if(c[i].Markup.Type=='Year'||c[i].Markup.Type=='YearNotMore'||c[i].Markup.Type=='YearNotLess'||c[i].Markup.Type=='Date'
               ||c[i].Markup.Type=='Integer'||c[i].Markup.Type=='Decimal') { js+=ctrl+".ValidateOnBlur({ Allow: '"+c[i].Markup.Type+"' });"; }
            }
         }
      }
      //eval(js);
   ...
   fnCheckAndSave: function(Action) {
      ...
      $("#dialog-form-validate-editable .validate-dialog").each(function() {
         ...
         if(Col.Markup!=undefined) {
            FUnique=(Col.Markup.IsUnique)?Col.Markup.IsUnique:0; Type=(Col.Markup.Type)?Col.Markup.Type:0;
         } else { FUnique=0; Type=0; }
         var Validity=Col.Validity;

         if(Type==="decimal") { child.val(child.val().replace(/,/g, ".")); }
         eval("child."+Validity);
         IsUniqueArr=(Col.Markup.IsUnique)?Col.Markup.IsUnique:IsUniqueArr;
         var Save=false;

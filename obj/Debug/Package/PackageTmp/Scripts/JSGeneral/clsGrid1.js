...
function ObjectsRegistrator() {
   var Obj=[]; this.ObjectsInside=Obj;
   this.SetObject=function(objName, oINST) {
      //var NewObject={ Name: objName, Instance: oINST }
      //Obj[Obj.length]={ Name: objName, Instance: oINST }
      Obj[objName]=oINST;
   }
   this.GetObject=function(objName) {
      //for(var i=0; i<Obj.length; i++) { if(objName==Obj[i].Name) return Obj[i].Instance; }
      return Obj[objName];
   }
}
function clsGrid(ServerData, objName) {
   var oINST=this;
      ...
      if(_SD.Grid.aoColumns.length>NewData.length) { //Jeigu nauju duomenu stulpeliu skaicius nesutampa su lenteles sk. patikrinam ir jei reikia pridedam trukstamus st
         ...
         for(var i=1; i<c.length; i++) {
            if(typeof (c[i].bVisible)!='undefined') {
               if(typeof _SD.Cols[i].Markup.Default!='undefined') {//ikisam trukstama
                  nd2=""; if(_SD.Cols[i].Markup.Default==='Today') nd2=fnGetTodayDateString();
                  NewData2.push(nd2);
               }
            }

   ...
   $.fn.megaselectlist=function (settings) {
      ...
      function IsSelected(compareList, compareValue) {
         ...
         for(var i=0; i<compareList.length; i++) {
            if($(compareList[i]).val()==compareValue) { found=true; }
         }
         return found;
      }
      return this.each(function () {
         ...
         if(config.isxhtml) { selectedAttribute=" selected=\"selected\""; }

         var originalId=$This.attr("id"), originalName=$This.attr("name");
         var label=$("label[for='"+originalId+"']"), labelText=$(label).text(); ;
         var selectedValue=$This.val(), selectedOptionValues=$This.find("option:selected");
         ...
         if(optgroups.length>0) {
            ...
            for(i=0; i<options.length; i++) {
               ...
               var x=IsSelected(selectedOptionValues, $(options[i]).val()), cnn="";
               if(x) { cnn=' class="currentitem"'; }
               replacementHtml+='<li rel="'+$(options[i]).val()+'"'+cnn+'>'+$(options[i]).text()+'</li>';
            }
            if(isHeaderOpen) { replacementHtml+='</ul></div>'; }
         }
         ...
         $This.remove(); $(label).hide().after(replacementHtml); $(label).remove();
         //-------------------------------------------------------------------------------------------------
         ...
         $("#"+classModifier+nextModifierNumber+" li").click(function () {
            var item=$(this), thisValue=$(item).attr("rel"); ;
            if(!config.multiple) {
         ...
         if(config.animate) {
            ...
            if(config.hideloading) {
               $("#"+classModifier+nextModifierNumber+" .megaselectlistoptions").attr("rel", optionHeight).css({ overflow: "hidden", height: "0px" });
            } else {
